...
const pviRates = {
  ...
  internal: '[1.6, 1.9, 2.15, 2.5][a]', // from "Xe hoạt động trong nội bộ cảng"
  specialized: '[1.8, 2.15, 2.4, 2.9][a]' // from "Xe tải gắn cẩu, cứu thương..."
};

// We will find each category block and replace the PVI property
for (const [key, value] of Object.entries(pviRates)) {
  const regex = new RegExp(`(${key}:\\s*{[\\s\\S]*?)PVI:\\s*\\(v,\\s*a\\)\\s*=>\\s*(?:{[^}]*}|\\[[^\\]]*\\]\\[a\\])`, 'g');
  content = content.replace(regex, `$1PVI: (v, a) => ${value}`);
}
